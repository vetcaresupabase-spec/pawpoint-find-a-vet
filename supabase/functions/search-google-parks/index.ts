...
function generateCacheKey(params: SearchParams): string {
  ...
  return `parks:${normalizedQuery}|${roundedLat}|${roundedLng}|${radius}`;
}

function transformGooglePlace(place: GooglePlace): TransformedPark {
  ...
  return {
    ...
    name: place.displayName?.text || "Unnamed Park",
...
async function searchGooglePlaces(
  params: SearchParams,
  apiKey: string
): Promise<TransformedPark[]> {
  const textQuery = params.query
    ? `dog park pet park off-leash ${params.query}`
    : "dog park pet park off-leash";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (cacheError && cacheError.code !== "PGRST116") {
      console.error("Parks cache lookup error:", cacheError);
    }

    // CACHE HIT
    if (cachedData) {
      console.log("Parks cache HIT for key:", cacheKey);
    ...
    console.log("Parks cache MISS for key:", cacheKey);
    ...
    if (insertError) {
      console.error("Parks cache insert error:", insertError);
    }
    ...
    console.error("Error in search-google-parks:", err);
    return new Response(
      JSON.stringify({
        success: false,
